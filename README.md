...
Bag of freebies:
  which can make the object detector receive better accuracy without increasing the inference cost. We call these methods that only
...
suppose we have a 3-level PPS. Suppose the conv5 (i.e. the last convolution layer) has 256 features map.
First, each feature map is pooled to become a one value. Then the size of the vector is (1, 256)
Then, each feature map is pooled to have 4 values. Then the size of the vector is (4, 256)
